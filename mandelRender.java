...
public class mandelRender {
    ...
    public static int[] mandelCalcRGBatXY(double x, double y, double w, double h, double offsetX, double offsetY, double zoom, double maxiter) {
        //calculate the mandelbrot value at x,y, given w and h, return 3 ints, R, G, and B
    ...
    public static void printMandel(int w, int h, double offsetX, double offsetY, double zoom, String outfile, double maxiter) {
    ...
    public static void main(String[] args) {
        ...
        if (width == "" || height == "") {
            System.out.println("Usage: MandelTest -w <width> -h <height> [-x <offsetX> -y <offsetY> -z <zoom> -o <filename(png)> -m <Z-Iterations>]");
            return;
        }
        int wint = Integer.parseInt(width);
        int hint = Integer.parseInt(height);
        double offsetXdouble = 0;    if (offsetX != "") { offsetXdouble = Double.parseDouble(offsetX);}
        double offsetYdouble = 0;    if (offsetY != "") { offsetYdouble = Double.parseDouble(offsetY);}
        double zoomdouble    = 1;    if (zoom    != "") { zoomdouble    = Double.parseDouble(zoom);}
        double maxiter       = 255*Math.sqrt(zoomdouble); if (iterstr != "") { maxiter       = Double.parseDouble(iterstr);}
        if (outfile == "") {
            outfile = "mandel.png";
        }
        System.out.println("Width:"+width+" Height:"+height+" offsetX:"+offsetXdouble+" offsetY:"+offsetYdouble+" zoom:"+zoomdouble+ " filename:"+outfile+" Z-Iterations:"+maxiter);
